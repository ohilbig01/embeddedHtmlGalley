 ...
 * Copyright (c) 2020-2025 State and University Library Hamburg 
		...
		<div class="entry_details">
			...
			{* sub-oh *}
                        <div class="item backlink">
                                <a href="{url page="article" op="view" path=$article->getId()}"><span class="fa fa-arrow-left"></span> {translate key="plugins.themes.modpub.backLink"}</a>
                        </div>
			...
			{if $plumPluginIsEnabled}
				<div class="item published">
					<div class="label"> {translate key="plugins.themes.modpub.plum.label"} </div>
			...
			<div class="item published">
				<section class="sub_item">
					...
					<h2 class="label"> {translate key="plugins.themes.modpub.dates.received"} </h2>
					<div class="value"> {$article->getDateSubmitted()|escape|date_format:$dateFormatShort} </div>
					{if $dateAccepted}
						<h2 class="label"> {translate key="plugins.themes.modpub.dates.accepted"} </h2>
			...
			{foreach from=$pubIdPlugins item=pubIdPlugin}
				{if $pubIdPlugin->getPubIdType() == 'doi' or $pubIdPlugin->getPubIdType() == 'other::urn'} {* sub-oh 20211010 no display of URN*}
					{continue}
				{/if}
